setInterval(function a() {
    ...
    document.querySelector(".time__filler").style.transform = `scaleX(${f})`;
    return a
}(), 5e3);
...
function touchHandler(i) {
    const j = i.changedTouches[0];
    const k = document.createEvent("MouseEvent");
    k.initMouseEvent({
        touchstart: "mousedown",
        touchmove: "mousemove",
        touchend: "mouseup"
    } [i.type], true, true, window, 1, j.screenX, j.screenY, j.clientX, j.clientY, false, false, false, false, 0, null);
...
function checkPos() {
    if (elem.offsetHeight + elem.offsetTop > window.innerHeight) elem.style.top = window.innerHeight - elem.offsetHeight + "px";
    if (elem.offsetWidth + elem.offsetLeft > window.innerWidth) elem.style.left = window.innerWidth - elem.offsetWidth + "px";
    if (elem.offsetLeft < -elem.offsetWidth / 2) elem.style.left = -Math.floor(elem.offsetWidth / 2) + "px";
    if (elem.offsetTop < -elem.offsetHeight / 2) elem.style.top = -Math.floor(elem.offsetHeight / 2) + "px"
}
